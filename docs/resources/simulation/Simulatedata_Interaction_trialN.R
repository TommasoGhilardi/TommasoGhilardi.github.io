...
random_slope_sd <- 4                     # Standard deviation of the random slope for trial number by subject
...
fixed_categorical_effect <- -20          # Fixed effect: difference between categorical levels (e.g., Spoon vs. Hammer)
fixed_continuous_effect <- 10            # Fixed effect of continuous variable (e.g., trial number)
interaction_effect <- 8                  # Interaction effect between categorical and continuous variables
residual_sd <- 5                        # Residual standard deviation
...
simulated_data = simulated_data |> 
  select(subject_id, categorical_condition, trial_number, dependent_variable) |>
  rename(condition = categorical_condition, performance = dependent_variable)
write.csv(simulated_data, "resources//simulation//simulated_data.csv", row.names = FALSE)
...
ggplot(simulated_data, aes(x = trial_number, y = performance, color = condition)) +
  geom_line() +
  geom_point() +
...
# Linear Model (LM)
mod_lm <- lm(performance ~  condition * trial_number, data = simulated_data)
summary(mod_lm)


# Mixed-Effects Model (LMM)
mod_mixed <- lmer(performance ~  condition * trial_number + 
...
Est <- estimate_expectation(mod_mixed, get_datagrid(mod_mixed, by = c('condition','trial_number')))
ggplot(Est, aes(x = trial_number, y = Predicted, color = condition, fill = condition)) +
